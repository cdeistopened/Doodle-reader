...
export const SubscribeModal: React.FC<SubscribeModalProps> = ({ isOpen, onClose, onSubscribe, error: externalError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUrl('');
      // Note: onClose is now called by parent on success
    } catch (err: any) {
      setInternalError(err.message || 'Could not find feed. Try checking the URL.');
    } finally {
      setLoading(false);
    }
  };
